...
def Nombre_Contraseña(ini, screen):
    ...
    # Inputs centrados
    input_nombre = InputBox((WIDTH//2 - input_width//2, HEIGHT//2 - 60, input_width, input_height), main_font)
    input_pass = InputBox((WIDTH//2 - input_width//2, HEIGHT//2 - 10, input_width, input_height), main_font)
    input_boxes = [input_nombre, input_pass]

    # Botones centrados respecto al ancho de los inputs
    btn_width = 120
    btn_height = 40
    spacing = 10  # espacio entre botones

    # Botón Generar a la derecha del input
    btn_generar = Button(
        (WIDTH//2 + input_width//2 + spacing, HEIGHT//2 - 10, btn_width, btn_height),
        "Generar", 
        (0,123,255)
    )
    ...
    btn_add = Button(
        (WIDTH//2 - btn_width - spacing, HEIGHT//2 + 50, btn_width, btn_height),
        "Añadir", 
        (25,135,84)
    )

    # Botón Cancelar centrado respecto al input
    btn_cancel = Button(
        (WIDTH//2 + spacing, HEIGHT//2 + 50, btn_width, btn_height),
        "Cancelar", 
        (108,117,125)
    )
    ...
    while running:
        ...
        screen.blit(main_font.render("Nombre:",True,COLOR_TEXT),(WIDTH//2-280, HEIGHT//2-50))
        screen.blit(main_font.render("Contraseña:",True,COLOR_TEXT),(WIDTH//2-280, HEIGHT//2))
